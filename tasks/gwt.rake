...
def deploy_gwt(repository_url, patch_version, repo_id)
  in_dir(product_path('gwt', 'gwt')) do
    ...
    sh "GWT_MAVEN_REPO_ID=#{repo_id} GWT_VERSION=2.8.2-p#{patch_version} GWT_MAVEN_REPO_URL=#{repository_url} JSINTEROP_VERSION=1.0.2-p#{patch_version} GWT_GPG_PASS=#{ENV['GPG_PASS']} GWT_DIST_FILE= ./maven/push-gwt.sh"
  end
end
