...
task 'elemental2:build' do
  ...
  in_dir(product_path) do
    version = elemental2_version
    sh "#{BAZEL_CMD} clean --expunge" unless ENV['BAZEL'] == 'no'
    ELEMENTAL2_MODULES.each do |artifact_key|
      sh "#{BAZEL_CMD} build //java/elemental2/#{artifact_key}:lib#{artifact_key}.jar" unless ENV['BAZEL'] == 'no'
      sh "#{BAZEL_CMD} build //java/elemental2/#{artifact_key}:lib#{artifact_key}-src.jar" unless ENV['BAZEL'] == 'no'
